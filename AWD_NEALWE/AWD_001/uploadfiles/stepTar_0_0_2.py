...
class Waf():
    def __init__(self,htmlOwner):
        self.htmlOwner = htmlOwner
        pass
    ...
    def Mkdir(self):
        try:
            os.system('su - {} -c "mkdir /tmp/backup/;mkdir /tmp/backup/website/;mkdir /tmp/backup/mysqlB/;mkdir /tmp/website_forback/;mkdir /tmp/logs/;mkdir /tmp/raw_logs/;"'.format(self.htmlOwner))
        except:
            print("backup error")
    ...
    def BackUp(self, remote_file_path, back_dir):
        try:
            command = 'su - {} -c "tar -zcvf {}backup/website/website_back_dir.tar.gz {}"'.format(self.htmlOwner, remote_file_path, back_dir.strip('\n'))
...
if __name__ == "__main__":
    ...
    # Useage: su - www-data -c "python /tmp/stepTar.py www-data /tmp/ /var/www/html"
    #  !!!  : su - www-data -c "python /tmp/stepTar.py www-data /tmp/ /var/www/html/*"  错的！！！！不能有/*

    htmlOwner = sys.argv[1]
    ...
    # time.sleep(10)
    os.system('su - {} -c "tar -zcvf  {}backup_nealwe.tar.gz {}backup"'.format(htmlOwner, remote_file_path, remote_file_path))
